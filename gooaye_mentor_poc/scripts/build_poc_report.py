...
def main() -> int:
    ...
    review_windows = read_json_opt(DATA / "evaluation" / "EP674" / "review_windows.json") or {}
    lines: list[str] = []
    lines += [
    ...
    lines += [
        ...
        f"- Review windows prepared: {len(review_windows.get('windows', [])) if isinstance(review_windows, dict) else 0}",
